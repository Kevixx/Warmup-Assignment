...
public class GraphSearch {
    ...
    public static Action[][] search(State initialState, Frontier frontier) {
        ...
        if (outputFixedSolution) {
            ...
            // Try to solve a few levels by hand, enter the found solutions below, and run
            // them:
            ...
            // state.getExpandedStates() - Returns an ArrayList<State> containing the states
            // reachable from the current state.
            // You should also take a look at Frontier.java to see which methods the
            // Frontier interface exposes
            //
            // printSearchStatus(expanded, frontier): As you can see below, the code will
            // print out status
            // (#expanded states, size of the frontier, #generated states, total time used)
            // for every 10000th node generated.
            // You should also make sure to print out these stats when a solution has been
            // found, so you can keep
            ...
            while (true) {
                ...
                if (state.isGoalState()) {
                    ...
                    for (State s : state.getExpandedStates()) {
                        if (!expanded.contains(s) && !frontier.contains(s)) {
                            frontier.add(s);
                        }
                    }
